...
var PresentationMode = {
  ...
  initialize: function presentationModeInitialize(options) {
    ...
    this.firstPage.addEventListener(
      "click",
      function () {
    ...
    this.lastPage.addEventListener(
      "click",
      function () {
        this.contextMenuOpen = false;
    ...
    this.pageRotateCw.addEventListener(
      "click",
      function () {
        this.contextMenuOpen = false;
    ...
    this.pageRotateCcw.addEventListener(
      "click",
      function () {
        this.contextMenuOpen = false;
...
var PageView = function pageView(
  ...
  this.reset = function pageViewReset(keepAnnotations) {
    ...
    for (var i = div.childNodes.length - 1; i >= 0; i--) {
      ...
      if (
        (this.zoomLayer && this.zoomLayer === node) ||
  ...
  this.draw = function pageviewDraw(callback) {
    ...
    if (!PDFJS.disableTextLayer) {
      ...
      textLayer = this.viewer.createTextLayerBuilder(
        textLayerDiv,
        this.id - 1,
        this.viewport
      );
...
var TextLayerBuilder = (function TextLayerBuilderClosure() {
  ...
  TextLayerBuilder.prototype = {
    ...
    setupRenderLayoutTimer: function TextLayerBuilder_setupRenderLayoutTimer() {
      ...
      var self = this;
      var lastScroll =
        this.lastScrollSource === null ? 0 : this.lastScrollSource.lastScroll;
    ...
    renderMatches: function TextLayerBuilder_renderMatches(matches) {
      ...
      var isSelectedPage =
        this.findController === null
          ? false
          : this.pageIdx === this.findController.selected.pageIdx;
...
var PDFViewerApplication = {
  ...
  load: function pdfViewLoad(pdfDocument, scale) {
    ...
    var showPreviousViewOnLoadPromise = Preferences.get(
      "showPreviousViewOnLoad"
    ).then(function (prefValue) {
      showPreviousViewOnLoad = prefValue;
    });
    var defaultZoomValue;
    var defaultZoomValuePromise = Preferences.get("defaultZoomValue").then(
      function (prefValue) {
  ...
  setHash: function pdfViewSetHash(hash) {
    ...
    if (hash.indexOf("=") >= 0) {
      ...
      if ("pagemode" in params) {
        if (
          params.pagemode === "thumbs" ||
          params.pagemode === "bookmarks" ||
          params.pagemode === "attachments"
        ) {
          this.switchSidebarView(
            params.pagemode === "bookmarks" ? "outline" : params.pagemode,
            true
          );
 ...
 * @implements {IRenderableView}
 */
var ThumbnailView = function thumbnailView(
  container,
  id,
  defaultViewport,
  linkService,
  ...
  this.setImage = function thumbnailViewSetImage(img) {
    ...
    if (
      Math.max(img.width / ctx.canvas.width, img.height / ctx.canvas.height) >
      MAX_SCALE_FACTOR
    ) {
      reducedWidth >>= 1;
      reducedHeight >>= 1;
      ...
      while (
        Math.max(
          reducedWidth / ctx.canvas.width,
          reducedHeight / ctx.canvas.height
        ) > MAX_SCALE_FACTOR
      ) {
        reducedImageCtx.drawImage(
          reducedImage,
...
window.addEventListener(
  ...
  function () {
    ...
    PDFHistory.updateCurrentBookmark(
...
window.addEventListener(
  ...
  function scalechange(evt) {
    ...
    if (
      !PDFViewerApplication.updateScaleControls &&
      (document.getElementById("pageWidthOption").selected ||
        document.getElementById("pageFitOption").selected ||
        document.getElementById("pageAutoOption").selected)
    ) {
